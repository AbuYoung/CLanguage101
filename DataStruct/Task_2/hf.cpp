// hf.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
struct haffNode {
	int weight;
	//权重，就是这个字符出现的个数；如果这个节点是个父节点的话
	//，就是两个子节点权重的和
	char data;
	//这个用来存字符本身，比如字符是'c'，data='c';
	struct haffNode *leftChild = NULL, *rightChild = NULL;
	//定义左右子节点指针
};

char code[256][50];
//用二维数组来储存字符的哈夫曼编码，其中第一维的下标表示的是这个字符的ASCII码
haffNode left[50];
//用来储存所有的左子节点
haffNode right[50];
//用来储存所有的右子节点
unsigned char saveChar = 0;
//用来保存二进制文件，因为char类型是1个字节，所以每8位储存一次
//,而且用unsigned无符号型，避免符号位干扰
unsigned char slidChar;
//定义一个字符备用，以应对可能需要的操作
...
void creatHaffman(haffNode *node, int length) {
	...
	haffNode parent;
	//生成父节点，因为我们的数组从大到小排序过了，所以数组最后面的就是最小的节点
	left[length] = node[length - 2], right[length] = node[length - 1];
	//定义子字节，用来存数组最后的两个节点
...
void coding(haffNode *node, char *keepCode, int length) {
	...
	if (node->leftChild == NULL || node->rightChild == NULL) {
		keepCode[length] = '\0';
		//给编码一个终止符，形成一个完整的字符串，方便拷贝，以防拷贝到之前的编码。
		strcpy_s(code[node->data - 0], keepCode);
		//调用strcpy函数拷贝字符串，其中code的下标用节点的字符(data)-0得到
		return;
	}
	keepCode[length] = '0';
...
int main() {
	int count[128] = {0};
	//用来统计字符个数，一开始清零，其中它的下标号表示这个字符的ASCII码
	char keepCode[50];
	//用于在生成编码的时候 临时储存编码
	//，真正储存编码的地方看代码最上面的code[][]数组
	...
	haffNode node[100];
	//用来储存哈夫曼节点，这里我申请100个空间事实上不需要那么大，大概需要26+26+20（26个英文字母大小写+标点符号）
	FILE *fpr = fopen("C:\\Users\\abuyo\\data.txt", "r");
	//读入文件，其中input.txt的路径你要自己设置，可以自己建立一个文本，写一些英文进去
	FILE *fpw = fopen("C:\\Users\\abuyo\\output1", "wb");
	//写入文件，wb是写入二进制文件，路径设置随意，但是要符合格式，写入的时候文件自动生成。
	FILE *fpr1 = fopen("C:\\Users\\abuyo\\output1", "rb");
	//用于解压缩时读入文件 ，rb是读入二进制文件
	FILE *fpw1 = fopen("C:\\Users\\abuyo\\output3.txt", "w");
	//用于解压缩时写入文件
	...
	//循环数组，因为ASCII表中有255个字符，
	//所以数组中有些字符是完全没有出现过的，我们要将出现过的存在charNode数组里。
	for (i = 0; i < 128; i++) {
		if (count[i] != 0) {
	...
	num = 0; //计数
	fseek(fpr, 0L,
		  0); //因为上面已经读过文件了，fpr指针已经向下移动，所以这边使用
			  // fseek函数将指针复原到离0（文件起始位置）0L（第0个字节）处
	while ((reder = fgetc(fpr)) != EOF) { //一个一个地读入字符
		for (i = 0; i < strlen(code[reder - 0]); i++) {
			saveChar |=
				code[reder - 0][i] -
				'0'; //让saveChar和编码中的每一位进行或操作，用字符的'1'-'0'，就可得到0000
					 // 0001.
			num++;
			if (num == 8) {
